...
function run_or_read(model, layer, nq, nl, tstep, obs_i, obs_j, max_weight, min_abs_coeff, fn; run=false)
    ...
    fn = format("data/{}_minabscoeff=2^{}_termcount.dat", fn, min_abs_coeff_exponent)
    expectation = 0
    terms_nbr = 0
...
function zte_exp(expvals, terms_nbr, times, trunc_coeffs; plotting=true, exact_expvals=nothing, abs_threshold=1e-16, fn="")
        try
    ...
    if plotting
        ...
        plot!(times, truncated_most_precise_cleaned, label="Truncated most precise", marker=:circle, guidefontsize=16, tickfontsize=12, legendfontsize=12,titlefontsize = 12, ms = 6, dpi= 300, legend=:topleft)
        # plot!(yscale=:log10)
        ...
        savefig(p, "evo_plot/"*fn*"_$(log2(trunc_coeffs[1]))_to_$(log2(trunc_coeffs[end]))_correction.png")
        display(p)
    end
